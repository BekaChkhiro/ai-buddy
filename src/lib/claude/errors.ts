...
export function parseAnthropicError(error: any): ClaudeError {
  ...
  if (error.status === 429) {
    ...
    return new RateLimitError(error.message || "Rate limit exceeded", retryAfter);
  }

  // Authentication error
  if (error.status === 401) {
    return new ClaudeError("Invalid API key", "AUTHENTICATION_ERROR", 401, error);
  }

  // Invalid request
  if (error.status === 400) {
    return new ClaudeError(error.message || "Invalid request", "INVALID_REQUEST", 400, error);
  }

  // Server error
  if (error.status >= 500) {
    return new ClaudeError("Anthropic API server error", "SERVER_ERROR", error.status, error);
  }
...
export async function retryWithBackoff<T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (claudeError.code === "AUTHENTICATION_ERROR" || claudeError.code === "INVALID_REQUEST") {
        throw claudeError;
      }
