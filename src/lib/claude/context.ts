...
import { MAX_CONTEXT_FILES, MAX_CONTEXT_FILE_SIZE, MAX_TOTAL_CONTEXT_SIZE, estimateTokens } from './config'
...
export async function buildChatContext(
  ...
  if (contextFiles && contextFiles.length > 0 && project.folderPath) {
    context.relevantFiles = await loadContextFiles(
      project.folderPath,
      contextFiles
    )
  }
 ...
 */
export function getSuggestedContextFiles(
  _projectPath: string,
  maxFiles: number = 5
): string[] {
